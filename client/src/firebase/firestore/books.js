...
export const getBookById = createAsyncThunk("books/getById", async (id) => {
  try {
...
export async function postBook(book) {
  ...
  if (!book.authors[0].length > 50)
    throw new Error("Author must be at most 50 characters");
  if (!regexAuthor.test(book.authors[0]))
    throw new Error("Only letters or points are allowed in Author");
